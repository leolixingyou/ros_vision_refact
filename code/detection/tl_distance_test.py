    ...
    if len(boxes) > 0 :
      for box_info in boxes:
        ...
        if remapping:
            ...
            cv2.rectangle(cur_img_2, c1_new, c2_new, _COLORS[cls_id], thickness=tl, lineType=cv2.LINE_AA)
            tf = max(tl - 1, 1)  # font thickness
            text = '{}:{:.1f}%'.format(class_names[cls_id], score * 100)
            t_size = cv2.getTextSize(text, 0, fontScale=tl / 3, thickness=tf)[0]
            c2_new = c1_new[0] + t_size[0], c1_new[1] - t_size[1] - 3
            cv2.rectangle(cur_img_2, c1_new, c2_new, _COLORS[cls_id], -1, cv2.LINE_AA)  # filled
...
if __name__ == '__main__':
    ...
    pred = Predictor(engine_path='/workspace/weights/trt/integrate_each_class_fp16_0314.trt')
    class_name = pred.class_names
    print(class_name)

    save_dir = f"/workspace/demo/runs/tl_distance/tl_{time.strftime('%Y_%m_%d_%H_%M_%S',time.localtime(time.time()))}/"
    ...
    img_path = '/workspace/ros_vision/output/'
    ftype = ['.png']
    ...
    for pfile in img_list:
        ...
        box_result_ori = pred.orig_inference(orig_img, conf=0.1, end2end=True,day_night=1)
        box_result_resized = pred.orig_inference(resized_img, conf=0.1, end2end=True,day_night=1)
