...
class BaseEngine(object):
    ...
    def zero_steam_inference(self, origin_img, conf=0.5, end2end=False):
        print('DETECTION...')
        t1 =time.time()
        img, ratio = preproc(origin_img, self.imgsz, self.mean, self.std)
        print('Pre-process is :',round((time.time()-t1)*1000,2),' ms')
        ...
        if dets is not None:
            ...
            # box_result = zero_vis(origin_img, final_boxes, final_scores, final_cls_inds,
            # origin_img, box_result = vis(origin_img, final_boxes, final_scores, final_cls_inds,
            box_result = zero_vis(final_boxes, final_scores, final_cls_inds,
                             conf=conf, class_names=self.class_names)
        print('Post-process is :',round((time.time()-t2)*1000,2),' ms')
...
def zero_vis(boxes, scores, cls_ids, conf=0.5, class_names=None):
    box_result = []
...
def vis(img, boxes, scores, cls_ids, conf=0.5, class_names=None):
    box_result = []
    tl = 3 or round(0.02 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
