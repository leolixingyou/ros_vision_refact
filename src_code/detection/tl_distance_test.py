...
if __name__ == '__main__':
    ...
    for cam_name in came_list:
        for scale in scale_factor:
            ...
            for idx, pfile in enumerate(img_list, start=1):
                fname = f'tl_{idx:03d}'
                log_dic[cam_name][f'x{scale}'][fname] = {}
                ...
                log_dic[cam_name][f'x{scale}'][fname]['ori detect'] = result_detected_ori
                log_dic[cam_name][f'x{scale}'][fname]['map detect'] = result_detected_map

                if result_detected_ori:
                    width_ori, height_ori, area_ori = box_info_cal(box_result_ori[0])
                    log_dic[cam_name][f'x{scale}'][fname]['ori box size'] = [width_ori, height_ori, area_ori]
                if result_detected_map:
                    width_resize, height_resize, area_resize = box_info_cal(box_result_resized[0])
                    log_dic[cam_name][f'x{scale}'][fname]['map box size'] = [width_resize, height_resize, area_resize]

    # Save log_dic to a CSV file
    csv_file_path = os.path.join(save_dir_ori, "detection_log.csv")
    with open(csv_file_path, mode='w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        writer.writerow(['Camera', 'Scale', 'Image_Name', 'Ori_Detect', 'Map_Detect', 'Ori_Box_Width', 'Ori_Box_Height', 'Ori_Box_Area', 'Map_Box_Width', 'Map_Box_Height', 'Map_Box_Area'])
        for cam_name, scales in log_dic.items():
            for scale, results in scales.items():
                for fname, detection in results.items():
                    ...
                    writer.writerow([cam_name, scale, fname, ori_detect, map_detect, ori_box_size[0], ori_box_size[1], ori_box_size[2], map_box_size[0], map_box_size[1], map_box_size[2]])
