...
class BaseEngine(object):
    ...
    def infer(self, image):
        ...
        if self.input_width==512 and self.input_height==256:
            seg = np.reshape(output[0], (-1, 3, 256, 512))
            ...
            seg = np.reshape(output[0], (-1, 3, 384, 640))
    ...
    def inference(self, img_path, conf=0.25):
        ...
        print('Seg_Process time:', round(self.ms, 2), 'ms')
    ...
    def process_output(self, outputs, img): 
        ...
        filtered_boxes, filtered_scores, filtered_indexes =  [], [], []
    ...
    def draw_segmentation(self, image, alpha = 0.5):
        image =  0 * np.ones((image.shape[0],image.shape[1],image.shape[2]), np.uint8)
    ...
    def draw_2D(self, image, alpha = 0.5, text=True):
        ...
        # cv2.putText(front_view, 'Device: RTX 3080 Ti', (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        # cv2.putText(front_view, f'Model: {self.model_name}+BiFPN', (10, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        # cv2.putText(front_view, 'Input Resolution: {}x{}'.format(self.input_width, self.input_height), (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        # cv2.putText(front_view, 'FPS: {:.2f}'.format(1000 / self.ms), (10, 190), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--engine', type=str, required=True, help='The TensorRT engine file path')
    parser.add_argument('--anchor', type=str, required=True, help='The anchors numpy file path')
    #parser.add_argument('--dtype', type=str, required=True, help='The anchors numpy file path')
    ...
    if input_path.endswith('.mp4'):
        ...
        while True:
            ...
            combined, t_ms = pred.draw_2D(frame)
        ...
        combined, t_ms = pred.draw_2D(image)
        cv2.imwrite(output_path, combined)
    
    cuda.Context.pop()
    print(f"Average processing time: {sum(t_ms)/len(t_ms)} ms / {1000/(sum(t_ms)/len(t_ms))} fps")
